...
import { bandClaim, isClaimState, type StateOverride } from '../../lib/childState/header';
import type { LiveChildState } from '../../hooks/useChildState';

// Under the header's state pills: where the shown state comes from
// (therapist override or the band), what the band currently reads, and in
// ?debug=1 the engine's provisional state and signal breakdown.
...
export default function BandStateIndicator({ live, debug, override, onBackToAuto, disabled }: Props) {
  ...
  const claim = bandClaim(result?.state, stale);
  ...
  if (live) {
    if (claim) {
      if (override) {
        const style = CLAIM_STYLE[claim];
        bandText = t('band_state_reads', { state: t(style.key) });
        bandDot = style.dot;
    ...
    } else if (isClaimState(result.provisional.state)) {
      // Enough valid signals to score, but thresholds are not signed off.
      bandText = `${t('band_state_label')}: ${t('band_state_withheld')}`;
    } else {
  ...
  return (
    <div className="mt-1 flex flex-col items-center gap-0.5" aria-live="polite">
      ...
      {(bandText || (claim && result?.flappingDetected)) && (
